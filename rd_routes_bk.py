...
def assign_multi(pid):
    ...
    for item in data:
        ...
        if uid.startswith('u_'):
            try: user_obj = User.query.get(int(uid[2:]))
            except: pass
            ...
            try:
                ...
                # If no linked user account, skip (can't create sub-assignment without user)
            except: pass
        elif uid.isdigit():
            try: user_obj = User.query.get(int(uid))
            except: pass
...
def project_logs(pid):
    from models.npd import RDProjectLog, NPDProject
    proj = NPDProject.query.get_or_404(pid)
    logs = RDProjectLog.query.filter_by(project_id=pid)\
                             .order_by(RDProjectLog.created_at.desc()).all()
    result = []
    for l in logs:
        result.append({
            'id'        : l.id,
...
@login_required
def sub_start(pid):
    """Executive starts their own timer"""
    from models.npd import RDSubAssignment, RDProjectLog
    sub = RDSubAssignment.query.filter_by(
        project_id=pid, user_id=current_user.id, is_active=True
    ).first()
    if not sub:
...
@login_required
def sub_end(pid):
    """Executive ends their own timer"""
    from models.npd import RDSubAssignment, RDProjectLog
    sub = RDSubAssignment.query.filter_by(
        project_id=pid, user_id=current_user.id, is_active=True
    ).first()
    ...
    db.session.add(RDProjectLog(
        ...
        user_id    = current_user.id,
        event      = 'finished',
        detail     = f"Finished by {current_user.full_name}" + (f" | Variant: {sub.variant_code}" if sub.variant_code else "") + f" | Duration: {sub.total_seconds//3600:02d}:{(sub.total_seconds%3600)//60:02d}:{sub.total_seconds%60:02d}",
        created_at = datetime.now(),
    ))
    db.session.commit()
    return jsonify(
