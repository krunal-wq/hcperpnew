...
class PurchaseOrder(db.Model):
    ...
    def is_editable(self):
        """POs can be edited at these statuses:
          • Draft             — freely editable (not yet submitted)
          • Pending Approval  — user can revise before manager approves; saving
                                automatically reverts to Draft for re-submission
          • Rejected          — manager rejected; user fixes and re-submits

        LOCKED at these statuses:
          • Approved          — manager approved; must Cancel first to change
          • Partial Received  — goods receipt in progress; immutable
          • Completed         — fully received; final state, immutable
          • Cancelled         — terminal; use Re-open button to bring back to Draft
